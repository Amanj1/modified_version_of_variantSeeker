...
//fastq_files = Channel.fromFilePairs("${params.fastq_dir}/*_{1,2,unpaired}.fq.gz",size:3)
fastq_files = Channel.fromFilePairs("${params.fastq_dir}/*.fastq.gz",size:1)
html_files = Channel.fromFilePairs("${params.html_dir}/html_{start,end}.txt",size:2)

fastq_files.into{reformat_S_in;
extraction_in}
...
process reformat_fastq_reads_S{
  ...
  script:
""" 
reformat.sh in=${reads[0]} out=${sample_id}_phase1_input_S.fasta
sleep 1s
...
blastn_phase1_with_fq = blastn_phase1_S.combine(extraction_in, by: 0)
...
process extract_fastq_reads_for_phase1{
  ...
  input:
  set sample_id, blastn_S, reads from blastn_phase1_with_fq 
  
  output:
  set sample_id, "${sample_id}_phase1_read_unpaired.fq.gz" into phase1_fq_reads
  script:
"""
cat ${blastn_S} | awk '{ print \$1 }' | awk '!seen[\$0]++' > list_S.txt
seqtk subseq ${reads[0]} list_S.txt | gzip > ${sample_id}_phase1_read_unpaired.fq.gz
sleep 10
rm list_S.txt
"""
}

phase1_fq_reads.into{asm_megahit_in;
reads_for_mapping}
...
process asm_megahit{
  ...
  input:
  set sample_id, read_un from asm_megahit_in
  ...
  script:
  """
  megahit -t ${task.cpus} --presets meta-sensitive -r ${read_un} --cpu-only  -o megahit_assembly
...
asm_for_mapping = asm_megahit_comb
...
process map_reads_to_contigs{
  ...
  input:
  set sample_id, megahit_contigs, read_single  from map_reads_to_contigs

  output:
  set sample_id, "${sample_id}_megahit_reads_to_contigs.sam.gz" into map_reads_to_contigs_out

  script:
  """
  bbwrap.sh ref=${megahit_contigs} in=${read_single} out=${sample_id}_megahit_reads_to_contigs.sam.gz usejni=t kfilter=22 subfilter=15 maxindel=80
  """
}
...
process asm_mapping_stats{
  ...
  input:
  set sample_id, megahit_mapping from asm_mapping_stats_in

  output:
  set sample_id, "${sample_id}_megahit_flagstat.txt" into asm_mapping_stats_out
...
process asm_per_ctg_coverage{
  ...
  input:
  set sample_id, megahit_mapping from asm_per_ctg_coverage_in

  output:
  set sample_id, "${sample_id}_megahit_reads_to_contigs.cov.txt" into asm_per_ctg_coverage_out
...
process asm_bedtools_parse_reads_mapped_to_contigs{
  ...
  input:
  set sample_id, megahit_mapping from parse_sam_mapping

  output:
  set sample_id, "${sample_id}_bedtools_megahit_reads_mapped_to_contigs.txt.gz" into parse_sam_mapping_out
...
process blastn_phase2_megahit{
  ...
  script:
""" 
/media/amanj/da2ae0b2-e696-4533-b28d-717986cfc6e1/Kronan_2023_variantseeker_B19_singleEndReads_230713/ncbi-blast-2.13.0+-x64-linux/ncbi-blast-2.13.0+/bin/blastn -db ${params.large_db_nucl} -query ${reads} -num_threads ${task.cpus} -outfmt 6 > "${sample_id}_blastn_phase2_megahit.out"
"""
}

blastn_phase2_results = blastn_phase2_megahit
...
process process_phase2_results{
  ...
  input:
  set sample_id, megahit_res from blastn_phase2_results 
  
  output:
  set sample_id, "${sample_id}_blastn_phase2_megahit_datamash_filt.txt", "${sample_id}_phase2_accession_num_list.txt" into phase2_processed_results
...
cat ${sample_id}_blastn_phase2_megahit_datamash_filt.txt | awk '{print \$2}' | awk '!seen[\$0]++' > ${sample_id}_phase2_accession_num_list.txt
...
process fetch_fasta_headers_local_DB{
  ...
  input:
  set sample_id, megahit_filt, acc_list from fetch_names_local_DB
...
process fetch_taxonomy_esearch{
  ...
  input:
  set sample_id, megahit_filt, acc_list from fetch_names_esearch
...
for line in \$(cat ${acc_list})
do
	esearch  -db nucleotide -query \$line | esummary > temp.txt
	...
	esearch  -db taxonomy -query "\$organism" | esummary > temp.txt
...
process preprocess_classification_data{
  ...
  input:
  set sample_id, taxonomy, megahit, acc_list from classification_data
